...
Une difficulté en modélisation phénoménologique est de discerner ce qui tient de la structure de ce qui tient du bruit. Lorsque l'on considère une structure comme du bruit, on est dans un cas de sousapprentissage. Lorsque, au contraire, on interprète du bruit comme une structure, on est en cas de surapprentissage. Les graphiques de la [figure @fig-mesapprentissage] présentent ces deux cas, avec au centre un cas d'apprentissage conforme.

```{r}
#| label: fig-mesapprentissage
#| fig-align: center
#| fig-cap: "Cas de figure de mésapprentissage. À gauche, sous-apprentissage. Au centre, apprentissage valide. À droite, surapprentissage."
#| echo: false
...
1. Le module **`mlr3`**, connu ancienment comme **`mlr`** (**M**achine **L**earning in **R**), occupe sensiblement le même créneau que **`caret`**, mais utilise plutôt une approche par objets connectés. Le livre « [Applied machine learning using mlr3 in R](https://mlr3book.mlr-org.com/) » disponible sur internet, offre une introduction à ce module avec des exemples pratiques.
...
#| fig-align: center
#| fig-cap: "<< Le... l'idée en arrière pour être... euh... simpliste, là c'est que c'est un peu de... euhmm... de la vitamine de vinyle.>> - Georges ([Les voisins](https://youtu.be/-RpYi_Vuviw?t=6m40s), une pièce de Claude Meunier)."
#| echo: false
...
#| out-width: 50%
#| fig-align: center
#| fig-cap: "Dimensions mesurés sur les dragons capturés."
#| echo: false
...
#| out-width: 100%
#| fig-align: center
#| fig-cap: "Les Ents, tiré du film le Seigneur des anneaux, qui prennent trop de temps avant de se décider - paradoxalement, les arbres de décisions sont dotés d'algorithmes rapides."
#| echo: false
...
#| out-width: 50%
#| fig-align: center
#| fig-cap: "Exemple d'arbre de décision, tiré du [blogue de Jeremy Jordon](https://www.jeremyjordan.me/decision-trees/)."
#| echo: false
...
Les réseaux neuronaux sont aptes à générer des sorties multiples. Nous désirons prédire une catégorie, et **`neuralnet`** ne s'occupe pas de les transformer de facto. Lors de la prédiction d'une catégorie, nous devons générer des sorties multiples qui permettront de décider de l'appartenance exclusive à une catégorie ou une autre. Nous avons abordé l'encodage catégoriel aux chapitres [-@sec-bios] et [-@sec-expl]. C'est ce que nous ferons ici.
...
Il n'existe pas de règle stricte sur le nombre de couche et le nombre de nœuds par couche. Il est néanmoins conseillé de générer d'abord un modèle simple, puis au besoin de le complexifier graduellement en terme de nombre de nœuds, puis de nombre de couches. Si vous désirez aller plus loin et utiliser keras, le module [`autokeras`](https://autokeras.com/), disponible seulement en Python, est conçu pour optimiser un modèle Keras.
...
La prédiction semble bien fonctionner en entraînement comme en test. Pour une application à un cas d'étude, disons que pour mon site j'ai des variables environnementales de valeurs du bloc 50, et que je cherche la dose optimale.
...
**Exercice**. Changez les valeurs des variables environnementales pour générer le tableau `environment_appl` avec des valeurs qui sortent du lot (voir figure @fig-ml_variables_env). Qu'observez-vous? Pourquoi?
