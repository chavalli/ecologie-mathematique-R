...
-   saurez effectuer une modélisation statistique linéaire simple, multiple et mixte, entre autres sur des catégories
...
Dans [*Principles and procedures of statistics: A biometrical approach*](https://ulaval.on.worldcat.org/oclc/1131250331), Steel, Torie et Dickey (1997) définissent les statistiques ainsi:
...
--- Bryan Howie (@bryan_howie) <a href="https://twitter.com/bryan_howie/status/1073054519808876544?ref_src=twsrc%5Etfw">13 décembre 2018</a>
...
```{r}
#| label: bios-iris-classic-quantile
...
1.  Les *p-values* indiquent l'ampleur de l'incompatibilité des données avec le modèle statistique
1.  Les *p-values* ne mesurent pas la probabilité que l'hypothèse étudiée soit vraie, ni la probabilité que les données ont été générées uniquement par la chance.
1.  Les conclusions scientifiques et décisions d'affaire ou politiques ne devraient pas être basées sur l'atteinte d'une *p-value* à un seuil spécifique.
1.  Une inférence appropriée demande un rapport complet et transparent.
1.  Une *p-value*, ou une signification statistique, ne mesure pas l'ampleur d'un effet ou l'importance d'un résultat.
1.  En tant que tel, une *p-value* n'offre pas une bonne mesure des évidences d'un modèle ou d'une hypothèse.
...
```{r}
#| label: bios-johnoliver
#| fig-align: center
#| fig-cap: "Un sketch humoristique de John Oliver sur le *p-hacking*, [Last week tonight, 2016](https://youtu.be/0Rnq1NpHdmw) (en anglais)"
#| echo: false
knitr::include_graphics("images/07_p-hacking.png")
```
...
## Les modèles statistiques{#sec-bios-models}
...
```{r}
#| label: bios-lm1-agridat-predict
...
```{r}
#| label: bios-lm1-agridat-predict-newdata
...
```{r}
#| label: bios-residuals-example-banana
set.seed(36164)
x <- 0:100
...
```{r}
#| label: bios-residuals-example-expand
set.seed(3984)
x <- 0:100
...
```{r}
#| label: bios-residuals-agridat-groups
...
```{r}
#| label: bios-dummy-coding-relevel
...
```{r}
#| label: bios-lasrosas-topo-levels
...
```{r}
#| label: bios-lasrosas-topo-boxplot
...
```{r}
#| label: bios-lasrosas-topo-model-matrix
...
```{r}
#| label: bios-lasrosas-topo-levels-lm
...
```{r}
#| label: bios-lasrosas-topo-levels-summ
...
```{r}
#| label: bios-encoding-ordinal-plot
...
```{r}
#| label: bios-multivariate-lm-data
head(lasrosas.corn)
```
...
```{r}
#| label: bios-multivariate-lm-scaled
...
```{r}
#| label: bios-multivariate-lm-ci-plot
intervals |> 
...
```{r}
#| label: bios-multivariate-lm-interaction
...
```{r}
#| label: bios-multivariate-modmat-interaction
...
```{r}
#| label: bios-karma-lm-interaction
tidy(lm(karma ~ sandales * bas, karma_df))
```

Le modèle est plus clair. Sans interaction, les effets sur le karma des bas et des sandales étaient négativement affectés par l'effet d'interaction `sandales:bas`, le karma étant poussé à la baisse par le [bas blanc dans vos sandales](https://youtu.be/_whvVXX0hCk?t=86).
...
```{r}
#| label: bios-multivariate-glm-hist
...
```{r}
#| label: bios-multivariate-glm-boxplot
...
```{r}
#| label: bios-multivariate-glm-model
...
```{r}
#| label: bios-multivariate-glm-ci
...
```{r}
#| label: bios-multivariate-glm-plot-effects
...
```{r}
#| label: bios-multivariate-glm-cars
mtcars |> slice_sample(n = 6)
```

```{r}
#| label: bios-multivariate-glm-cars-plot
mtcars |> 
...
```{r}
#| label: bios-multivariate-glm-cars-model
...
```{r}
#| label: bios-multivariate-nls-data
engelstad.nitro |> slice_sample(n = 10)
```

```{r}
#| label: bios-multivariate-nls-plot
engelstad.nitro |> 
...
```{r}
#| label: biostats-multivariate-nls-miscth-explore
...
```{r}
#| label: bios-multivariate-nls-model
modnl_1 <-  nls(yield ~ A * (1 - exp(-R*(E + nitro))),
                data = engelstad.nitro,
...
```{r}
#| label: bios-multivariate-nls-summary
summary(modnl_1)
```
...
```{r}
#| label: bios-multivariate-nls-resultat-plot
x <- seq(0, 350, by = 5)
y <- mitscherlich_f(x,
...
```{r}
#| label: bios-multivariate-nls-residuals
tibble(res = residuals(modnl_1)) |> 
    ggplot(aes(x = res)) + geom_histogram(bins = 20)
```

```{r}
#| label: bios-multivariate-nls-residuals2
...
```{r}
#| label: bios-multivariate-nlme-model
library("nlme")
...
```{r}
#| label: bios-multivariate-nlme-model-summ
summary(mmodlin_1)
```
...
```{r}
#| label: bios-multivariate-nlme-nl-model
...
```{r}
#| label: bios-multivariate-nlme-nl-plot
engelstad.nitro |> 
  ggplot(aes(x = nitro, y = yield)) +
  facet_grid(year ~ loc) +
...
**Note**. L'interprétation de *p-values* sur les modèles mixtes est controversée. À ce sujet, Douglas Bates a écrit une longue lettre à la communauté de développement du module `lme4`, une alternative à `nlme`, qui remet en cause l'utilisation des p-values, [ici](https://stat.ethz.ch/pipermail/r-help/2006-May/094765.html). De plus en plus, pour les modèles mixtes, on se tourne vers les statistiques bayésiennes, couvertes dans le [chapitre @sec-bayes] avec le module greta. Mais en ce qui a trait aux modèles mixtes, le module [`brms`](https://github.com/paul-buerkner/brms) automatise bien des aspects de l'approche bayésienne.

### Aller plus loin

#### Statistiques générales:

-   [The analysis of biological data](https://ulaval.on.worldcat.org/oclc/1178980667)

#### Statistiques avec R

-   Disponibles en version électronique à la bibliothèque de l'Université Laval:
    -   Introduction aux statistiques avec R: [Introductory statistics with R](https://ulaval.on.worldcat.org/oclc/759004956)
    -   Approfondir les statistiques avec R: [The R Book, Third edition](https://ulaval.on.worldcat.org/oclc/1323328970)
    -   Approfondir les modèles à effets mixtes avec R: [Mixed Effects Models and Extensions in Ecology with R](https://ulaval.on.worldcat.org/oclc/690380460)
