...
- dplyr: manipuler les tableaux (sélection, filtres)
...
- Inscriptions non systématiques
...
- csv: texte brut (encodage recommandé: UTF-8)
...
Avec le symbole *pipe*, `%>%` (raccourci: Ctrl + Shift + M), qui se lit comme *puis*.

```{r}
chicoute %>%
  select(Site, Latitude_m, Longitude_m) # ou simplement select
```

## À propos de `%>%`

Le symbole *pipe*, **`%>%`** (raccourci: Ctrl + Alt + M) fait partie du module `magrittr`, inclus dans tidyverse. En 2021, l'équivalent a été intégré à R-base et prend la forme **`|>`**. Si vous le souhaitez, vous pouvez l'activer par défaut dans les options de RStudio et l'utiliser avec Ctrl + Shift + M.
...
```{r}
chicoute %>% 
 select(Site, Latitude_m, Longitude_m) %>% head(5)
...
Remarquez `%in%`, nouvel opérateur logique, qui signifie *`==` à l'un de ...*.
...
Les % en une seule variable? Utiliser `pivot_longer()`. Avant d'utiliser cette opération, assurez-vous que chaque ligne ait un identifiant unique.

```{r}
chicoute_long <- chicoute |>   
  select(CodeTourbiere, N_pourc, P_pourc, K_pourc) |>   
  mutate(ID = 1:n()) |>   # mutate ajoute une colonne au tableau
  # 1:n() peut être remplacé par cur_group_rows()
  pivot_longer(names_to = "element", values_to = "concentration", cols = c(-CodeTourbiere, -ID))
chicoute_long |>   slice_sample(n = 6)
```
...
J'ai des informations supplémentaires à propos des tourbières (colonne `CodeTourbiere`). Comment les combiner avec l'information existante?
...
# Note : On ajoute na.rm = TRUE pour retirer les valeurs nulles et éviter des problèmes :
sum(chicoute$Rendement_g_5m2)
```
...
```{r}
chicoute |>  
  pull(Rendement_g_5m2) |>  # pull extrait le vecteur, comme avec $, alors que select génère un tableau
  sum(na.rm = TRUE)
```
...
Les sommaires groupés avec plusieurs colonnes.
...
Espèces menacées, données issues de l'Union Internationale pour la Conservation de la Nature (UICN, [distribuées par l'OCDE](https://stats.oecd.org/Index.aspx?DataSetCode=WILD_LIFE)).
...
Note : Au lieu d'utiliser `arrange (desc(n_critical_species)) |>  slice_head(n = 10)`, on aurait simplement pu utiliser la commande `slice_max(n_critical_plants, n = 10)` pour le même résultat.
...
especes_menacees |> 
  ...
  group_by(Country)  |> 
  summarise(n_critical_species = sum(Value)) |> 
  slice_max(n_critical_species, n = 10)
```
...
-   [Chapitre 3 des notes de cours](https://chavalli.github.io/ecologie-mathematique-R/03-tableaux.html#op%C3%A9rations-sur-les-tableaux)
-   
