...
- serez en mesure d'effectuer des calculs permettant de mesurer des différences entre des observations, des groupes d'observation ou des variables observées
...
## Partitionnement (1/)
Catégoriser des objets qui n'appartiennent *a priori* à aucune catégorie pour faire en sorte que les différences entre les groupes soient plus grande que les différences entre les objets d'un groupe.
...
## Analyse en composantes principales: l'inertie (5/)
...
```{r}
library("ade4")
iris_pca <- dudi.pca(df = iris |> dplyr::select(-Species),
...
**Triplot de corrélation**. Les angles entre les variables représentent leurs corrélation, que les variables soient réponse ou explicative, ou entre variables réponses et variables explicatives. Les distances entre les objets sur le triplot ne sont pas des approximations de leur distance euclidienne.
