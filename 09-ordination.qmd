...
3. L'**ordination** vise par l'intermédiaire de techniques de réduction d'axe à mettre de l'ordre dans des données dont le nombre élevé de variables peut amener à des difficultés d'appréciation et d'interprétation. 

```{r}
#| label: apo-load-tidyverse
...
La racine carrée permet, pour certains indices de similarité, d'obtenir des propriétés euclidiennes. Pour plus de détails, voyez le tableau 7.2 de [Legendre et Legendre (2012)](https://www.elsevier.com/books/numerical-ecology/legendre/978-0-444-53868-0).
...
#### Objets: Occurrence (présence-absence)
...
```{r}
#| label: apo-dist-iris-comparaison
#| fig-height: 5
#| fig-width: 5
#| fig-align: center
...
```{r}
#| label: apo-dist-comparaison-plot
#| fig-align: center
...
1. *Méthodes hiérarchique et non hiérarchiques*. Dans un partitionnement hiérarchique, l'ensemble des objets forme un groupe, comprenant des sous-regroupements, des sous-sous-regroupements, etc., dont les objets forment l'ultime partitionnement. On pourra alors identifier comment se décline un partitionnement. À l'inverse, un partitionnement non-hiérarchique des algorithmes permettent de créer les groupes non hiérarchisés les plus différents que possible.
...
1. L’algorithme commence par placer une nombre *k* de centroides au hasard dans l'espace d'un nombre *p* de variables (vous devez fixer *k*, et *p* est le nombre de colonnes de vos données).
...
![](https://media.giphy.com/media/12vVAGkaqHUqCQ/giphy.gif){fig-align="center"}
...
```{r}
#| label: apo-cluster-kmeans-mcinnes
...
```{r}
#| label: apo-cluster-kmeans-mcinnes-plot
...
```{r}
#| label: apo-cluster-kmeans-mcinnes-silhouette
#| fig-align: center
...
```{r}
#| label: apo-cluster-kmeans-mcinnes-6groupes
#| fig-align: center
...
```{r}
#| label: apo-cluster-kmeans-mcinnes-silhouette-plot
#| fig-height: 30
#| fig-width: 8
#| fig-align: center
...
![](images/07_dbscan_1.svg){width=600 fig-align="center"}

2. Les noyaux distancés de *d* ou moins sont connectés entre eux en englobant les bordures.

![](images/07_dbscan_2.svg){width=600 fig-align="center"}
...
```{r}
#| label: apo-cluster-dbscan-mcinnes
...
```{r}
#| label: apo-cluster-dbscan-mcinnes-5groupes
#| fig-align: center
...
```{r}
#| label:  apo-cluster-hierarch-mcinnes
#| fig-align: center
...
```{r}
#| label: apo-cluster-hierarch-mcinnes-best
...
```{r}
#| label: apo-cluster-hierarch-mcinnes-plot
#| fig-width: 15
#| fig-height: 5
#| fig-align: center
...
```{r}
#| label: apo-cluster-hierarch-mcinnes-ngroupes
#| fig-align: center
...
```{r}
#| label: apo-cluster-hierarch-mcinnes-5groupes
#| fig-width: 15
#| fig-height: 5
#| fig-align: center
...
```{r}
#| label: apo-cluster-hierarch-mcinnes-5groupes-plot
#| fig-align: center
...
```{r}
#| label:  apo-cluster-hbdscan-mcinnes
#| fig-width: 15
#| fig-height: 7
#| fig-align: center
...
```{r}
#| label: apo-cluster-hbdscan-mcinnes-plot
#| fig-align: center
...
#| label: apo-cluster-hbdscan-mcinnes-plot2
#| fig-width: 15
#| fig-height: 7
#| fig-align: center
...
```{r}
#| label: apo-cluster-hbdscan-mcinnes-plot3
#| fig-width: 8
#| fig-height: 8
#| fig-align: center
...
Au [chapitre @sec-visual], nous avons vu avec le jeu de données "datasaurus" que la visualisation peut permettre de détecter des structures en segmentant les données selon des groupes.

![](images/07_datasaurus_mix.png){width=400 fig-align="center"}
...
```{r}
#| label: apo-iris-sepal-multinormalite
...
```{r}
#| label: apo-iris-sepal-eigen-scaled
...
```{r}
#| label: apo-iris-sepal-centroide

centroid <- setosa_sepal %>% apply(., 2, mean)
```

```{r }
#| label: apo-iris-sepal-plot
#| fig-align: center
...
![](images/07_biplot-meteo-sc2.svg){fig-align="center"}
...
1. les distances euclidiennes entre les scores sont des approximations des distances euclidiennes dans l'espace multidimensionnel,
...
Comme nous l'avons vu précedemment, les données de concentration sont de type *compositionnelles*. Les données compositionnelles du tableau `varechem` mériteraient d'être transformées ([Aitchison et Greenacre, 2002](http://doi.wiley.com/10.1111/1467-9876.00275)). Utilisons les log-ratios centrés (*clr*).

```{r}
#| label: apo-clr-varechem
...
```{r}
#| label: apo-varechem-pca-summary
...
```{r}
#| label: apo-varechem-pca-inertie
...
```{r}
#| label: apo-varechem-pca-bs-func
...
```{r}
#| label: apo-varechem-pca-bs-plot
#| fig-align: center
...
```{r}
#| label: apo-varechem-pca-loadings
...
```{r}
#| label: apo-varechem-pca-biplots
#| fig-align: center
...
```{r}
#| label: apo-varechem-pca-al-fe-mo
#| fig-align: center
...
```{r}
#| label: apo-varespec-cca-analysis
#| fig-width: 10
#| fig-height: 4
#| fig-align: center
...
Les représentations biplot d'analyse de correspondance peuvent prendre la forme d'un boomerang, en particulier celles qui sont basées sur des données d'occurrence. Le tableau suivant initialement de [Chessel et al. (1987)](http://pbil.univ-lyon1.fr/R/pdf/pps047.pdf) et est distribué dans le module ade4.

```{r}
#| label: apo-doubs-cca-biplot
#| fig-align: center
...
```{r}
#| label: aop-pomd-abun-compute-nmds
#| fig-align: center
...
![](images/07_ionome-revisited-figure4d.png){alt=" width=600 fig-align="center"}
...
Il s'agit toutefois d'une visualisation pour le diagnostic davantage que pour la publication. Si l'objectif est la publication, vous pourriez utiliser la fonction `plotDA` que j'ai conçue à cet effet. J'ai aussi conçu une [fonction similaire qui utilise le module graphique de base de R](https://raw.githubusercontent.com/essicolo/AgFun/master/plotDA_trad.R).

```{r}
#| label: apo-lda-biplot
#| fig-align: center
...
<div class="alert alert-block alert-success">**Excercice**. Si l'on effectuait l'ADL sur notre dauphin, avec la colonne `anatomy` comme  variable de regroupement, qu'obtiendrions-nous? Si l'on considère la nageoire codale (queue) comme faisant partie du corps? Quelles sont les limitations?</div>
...
```{r}
#| label: apo-vare-rda-perm-resultats
